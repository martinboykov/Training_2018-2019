...
// 7.1 Better debugging option (we must set debugging option in cli
// every time we want to use it)
// set DEBUG=app:db or set DEBUG=app:startup, app:db or set DEBUG=app:*
if (app.get('env') === 'development') {
    app.use(morgan('tiny'));
    startupDebugger('Morgan middleware is enabled');
}
// 7.1.1 db debugger (we must set debugging option in cli
// every time we want to use it)
// set DEBUG=app:db or set DEBUG=app:startup, app:db or set DEBUG=app:*

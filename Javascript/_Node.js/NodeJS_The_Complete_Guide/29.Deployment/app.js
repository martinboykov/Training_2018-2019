...
// const MONGODB_URI = `mongodb+srv://${process.env.MONGO_USER}:${process.env.MONGO_PASSWORD}@nodejsmongotest-h6qbj.mongodb.net/${process.env.MONGO_DEFAULT_DATABASE}`;
// const MONGODB_URI = `mongodb+srv://martinboykov:abcd995511@nodejsmongotest-h6qbj.mongodb.net/test?retryWrites=true`;
const MONGODB_URI = `mongodb+srv://${process.env.MONGO_USER}:${process.env.MONGO_PASSWORD}@cluster0-ld5wg.mongodb.net/${process.env.MONGO_DEFAULT_DATABASE}`;
...
app.use(session({
    ...
    secret: `${process.env.SESSION_SECRET}`, // !!!! SECRET MUST BE IN CONFIG ENV_VARIABLES
...
const accessFileStream = fs.createWriteStream(
    ...
    { flags: 'a' } // means that new data will be appended to the existing file
);
app.use(morgan('combined', { stream: accessFileStream })); // some hosting providers have logging, so this one wouldnt be necessasry
...
mongoose
    ...
    .then(async (connection) => {
        ...
        // start the node server in https mode (some host providers (for example: Heroku, AWS, Google Cloud) are doing this for you)

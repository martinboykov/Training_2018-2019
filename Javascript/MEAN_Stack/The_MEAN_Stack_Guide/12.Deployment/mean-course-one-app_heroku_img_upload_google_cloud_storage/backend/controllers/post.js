...
const postPost = async (req, res, next) => {
  ...
  // const imagePath = req.file.cloudStoragePublicUrl;
  ...
  const post = new Post({ // mongo driver adds _id behind the scene,  before it is saved to MongoDB
    ...
    imagePath: req.file.cloudStoragePublicUrl, // req.file.filename is provided by multer
...
const putPost = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) { // if we change the img
      ...
      const filename = post.imagePath.split('https://storage.googleapis.com/mean-app-uploaded-images/')[1];

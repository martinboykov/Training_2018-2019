...
const putPost = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) { // if we change the img
      ...
      // ---------------------------------------------------------
      // FILE DELETION DOESNT WORK ON AWS (no rights or smth)
      // ---------------------------------------------------------
      // console.log(path.join(__dirname, 'images', req.post.imagePath.split(url)[1]));
      // deleteImage(path.join(__dirname, 'images', req.post.imagePath.split(url)[1]));
...
const deletePost = async (req, res, next) => {
  try {
    ...
    if (!post) {
      ...
      // ---------------------------------------------------------
      // FILE DELETION DOESNT WORK ON AWS (no rights or CORS on angular server)
      // ---------------------------------------------------------
      // console.log(path.join(__dirname, 'images', req.post.imagePath.split(url)[1]));
      // deleteImage(path.join(__dirname, 'images', req.post.imagePath.split(url)[1]));
